...
function popularHorarios(geracoes) {
    ...
    for (let i = 0; i < geracoes; i++) {
            ...
            periodos.forEach(({ id, disciplinas }) => {
                disciplinas.forEach(disciplina => {
                    if (!cargaDisciplinas.has(disciplina)) {
                        cargaDisciplinas.set(disciplina, { professor, periodo: id, count: 0 });
                    }
                });
            });
        });
        ...
        cargaDisciplinas.forEach((info, disciplina) => {
            ...
            let horariosDisponiveis = Array.from({ length: 20 }, (_, i) => baseIndex + i);
            ...
            while (alocacoesFeitas < 5 && horariosDisponiveis.length > 0) {
                ...
                if (!horarios[horarioSorteado]) {
                    horarios[horarioSorteado] = { professor: info.professor, disciplina };
                    info.count++;
                    alocacoesFeitas++;
                }
            }
        ...
        horarios.forEach((horario, i) => {
            if (!horario) {
                let disciplinasFaltantes = Array.from(cargaDisciplinas.entries())
                .filter(([_, info]) => info.count < 5);
                
                if (disciplinasFaltantes.length > 0) {
                    let [disciplina, info] = disciplinasFaltantes[Math.floor(Math.random() * disciplinasFaltantes.length)];
        ...
        scheduleDataSet.push( horarios.map(h => `${h.professor}-${h.disciplina}`));
    }
...
console.log(scheduleDataSet);
  ...
  scheduleDataSet.forEach((disciplineSchedule) => {
    const row = document.createElement("tr");
    row.style.border = "1px solid black"; // Add border to rows
    disciplineSchedule.forEach((discipline) => {
