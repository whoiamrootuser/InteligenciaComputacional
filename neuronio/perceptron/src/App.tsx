...
const App: React.FC = () => {
  ...
  const [gridState, setGridState] = useState<number[]>(
    new Array(NUM_INPUT_FEATURES).fill(0),
  );
    ...
    setTimeout(() => {
      ...
      for (let epoch = 0; epoch < EPOCHS; epoch++) {
        ...
        TRAINING_DATA.forEach((instance) => {
          for (let i = 0; i < NUM_OUTPUT_BITS; i++) {
            const currentPrediction = currentPerceptrons[i].predict(
              instance.input,
            );
            ...
            console.log(`Epoch ${epoch + 1}, Weights: ${currentPerceptrons[i].weights}`);
          }
        });
      ...
      if (allConverged) {
        setTrainingMessage(
          `Treinamento concluído. Convergido em ${totalEpochsRun} épocas.`,
        );
      } else {
        setTrainingMessage(
          `Treinamento concluído após ${totalEpochsRun} épocas. Pode não ter convergido completamente.`,
        );
      }
    }, 100);
  }, [perceptrons]);
  ...
  const handlePredict = useCallback(() => {
    if (!isTrained || perceptrons.length === 0) {
      setRecognizedDigit(
        'Rede ainda não treinada ou treinamento em andamento.',
      );
      setPredictedOutput(null);
      return;
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-100 via-slate-50 to-gray-100 text-slate-800 flex flex-col items-center p-4 sm:p-8 selection:bg-sky-500 selection:text-white">
      <header className="mb-6 sm:mb-10 text-center">
        ...
        <p className="text-slate-600 mt-2 text-sm sm:text-lg max-w-2xl">
          Desenhe um dígito de 0 a 9 na tabela e clique em "Reconhecer Dígito"
      ...
      <footer className="mt-10 sm:mt-16 text-center text-slate-600 text-xs sm:text-sm">
        <p>
          Utilizando {NUM_OUTPUT_BITS} Perceptrons, cada um aprendendo uma saída
          (T).
        </p>
        <p>
          Bias Inicial:{' '}
          {PERCEPTRON_INITIAL_BIAS === undefined
            ? 'Aleatório'
            : PERCEPTRON_INITIAL_BIAS}
          , Limiar: {PERCEPTRON_ACTIVATION_THRESHOLD}
        </p>
      </footer>
    </div>
  );
