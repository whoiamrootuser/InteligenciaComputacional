...
const OutputDisplay: React.FC<OutputDisplayProps> = ({ trainingMessage, predictedOutput, recognizedDigit, isTrained }) => {
  let trainingMessageColor = 'text-yellow-600';
  if (trainingMessage.toLowerCase().includes('complete') && !trainingMessage.toLowerCase().includes('may not')) {
    trainingMessageColor = 'text-green-600'; 
  } else if (trainingMessage.toLowerCase().includes('error') || trainingMessage.toLowerCase().includes('failed')) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold mb-3 text-sky-600">Status do treinamento</h2>
        <div className="bg-slate-200/70 p-4 rounded-lg min-h-[4rem] flex items-center justify-center shadow-inner">
          <p className={`text-center text-sm sm:text-base ${trainingMessageColor}`}>
        ...
        <div className="bg-slate-200/70 p-4 rounded-lg min-h-[8rem] flex flex-col items-center justify-center space-y-2 shadow-inner">
          {predictedOutput && (
            <p className="text-slate-700 text-xs sm:text-sm">
              Padrão de Saída: <span className="font-mono text-sky-600 text-sm sm:text-base">[{predictedOutput.join(', ')}]</span> 
            </p>
          )}
          {recognizedDigit !== null ? (
            <p className="text-3xl sm:text-4xl font-bold">
              {typeof recognizedDigit === 'number' ? 
                ...
                <span className="text-yellow-600 text-xl sm:text-2xl">{recognizedDigit}</span>
              }
            </p>
          ) : (
            <p className="text-slate-500 text-sm sm:text-base">{isTrained ? 'Desenhe um dígito e clique em "Reconhecer Digito"' : 'Treine a rede primeiro'}</p>
          )}
        </div>
      </div>
    </div>
...
export const TrainingDataDisplay: React.FC = () => {
  ...
  return (
    <div className="mt-6 bg-slate-200/50 p-4 rounded-lg shadow-md">
      ...
      >
        <span>Visualizar dados para treinamento ({TRAINING_DATA.length} amostras)</span>
          ...
          {TRAINING_DATA.map((item, index) => (
            <div key={index} className="p-3 bg-white/80 rounded-md shadow">
              <p className="font-mono">
                <span className="text-sky-600 font-bold">Digit: {item.digit}</span>
              </p>
